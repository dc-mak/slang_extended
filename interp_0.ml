...
let rec interpret (e, env, store) = 
    ...
    | Seq (e :: rest)  -> let (v,  store1) = interpret(e, env, store) in
                          let _ = print_string ("intermd> " ^ string_of_value v ^ "\n") in
    ...
    | UnaryOp(op, e)   -> let (v, store') = interpret(e, env, store) in
                          (do_unary(op, v), store')
    (* short circuit evaluation *)
    | Op(e1, AND, e2)  -> interpret (If(e1, e2, Boolean false), env, store)
    | Op(e1, OR, e2)   -> interpret (If(e1, Boolean true,  e2), env, store)
