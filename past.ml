...
type oper = ADD | SUB | MUL | DIV | MOD | LT | AND | OR | EQ | EQB | EQI
...
type expr = 
       ...
       | LetRecFun of loc * var * lambda * expr
...
let string2_of_loc loc = 
    "L" ^ (string_of_int (loc.Lexing.pos_lnum)) ^ ", " ^ 
    "P" ^ (string_of_int ((loc.Lexing.pos_cnum - loc.Lexing.pos_bol) + 1))
